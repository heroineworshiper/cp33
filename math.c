...
{
	while(angle < 0) angle += 360 * 256;
	while(angle >= 360 * 256) angle -= 360 * 256;
...
// 180 - 270
	if(angle <= 270 * 256)
	{
		int index = (angle - 180 * 256) / 90;
		return tan_table[index];
	}
	else
// 270 - 360
	{
		int index = 256 - (angle - 270 * 256) / 90;
		return -tan_table[index];
	}
}
...
{	
	while(x < 0) x += 360 * 256;
	while(x >= 360 * 256) x -= 360 * 256;

// 270 - 360
	if(x >= 270 * 256)
	{
		int x_fraction = (360 * 256 - x) / 90;
		return -sin_table[x_fraction];
	}
	else
// 180 - 270
	if(x >= 180 * 256)
	{
		int x_fraction = (x - 180 * 256) / 90;
		return -sin_table[x_fraction];
	}
	else
// 90 - 180
...
void print_matrix(matrix_t *mat)
{
	...
	for(i = 0; i < mat->rows; i++)
	{
		...
		{
			print_fixed(MATRIX_ENTRY(*mat, i, j));
		}
		print_lf();
	}
}
...
void print_vector(vector_t *vec)
{
	...
	{
		print_fixed(VECTOR_ENTRY(*vec, i));
	}
	print_lf();
}
...
{
	int result = new_angle - old_angle;
	if(result > 180 * 256) result -= 360 * 256;
	else
	if(result < -180 * 256) result += 360 * 256;
	return result;
}
...
{
	if(angle > 180 * 256) angle -= 360 * 256;
	if(angle < -180 * 256) angle += 360 * 256;
	return angle;
}
	...
	int y2)
{
	...
	*angle = atan2_fixed(y2 - y1, x2 - x1);
	...
	{
		*distance = (x2 - x1) * 256 / cos_fixed(*angle);
	}
	else
	{
		*distance = (y2 - y1) * 256 / sin_fixed(*angle);
	}
...
{
	ptr->prev = kmalloc(buffer_size * sizeof(int), 1);
